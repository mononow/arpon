  ...
  handler({ target, development, simulator, platform }) {
    ...
    platforms.forEach((plat) => {
      ...
      const cmd = [
        ...
        ADD_SIMULATOR && "Arpon_SIMULATOR=true",
        (choseAllPlatforms || platforms.length > 1) && "BUILD_ALL=true",
        `PLATFORM=${plat}`,
        `webpack --env PLATFORM=${plat} --config "${getWebpackConfigPath(
          "prod",
        )}"`,
      ]
        .filter(Boolean)
      ...
      if (ADD_SIMULATOR) {
        console.log(pico.yellow("  Adding the Arpon simulator to the bundle"));
      }
