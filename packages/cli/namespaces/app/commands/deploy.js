    ...
    if (usage === true) {
      ...
      console.log(
        `    Build for MP35(P): ${pico.cyan(
          `manydots app deploy -p MP35P -f /data/users/10004/apps/`,
        )}\n`,
      );
      console.log(`    Build with Q92: ${pico.cyan(`manydots app deploy -p Q92`)}\n`);
      console.log(`    Build with XCB: ${pico.cyan(`manydots app deploy -t xcb`)}\n`);
      return;
    }
    const { id } = PKG.manydots;
