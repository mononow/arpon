...
/**
  ...
  Arpon infrastructure looger for webpack:

  Arpon  compiled successfully in 1.32s
...
const withTapCompiler =
  (compiler: webpack.Compiler, name: string) => (type: høøks, fn: Fn) => {
 ...
 * Arpon Logger class
 */
export default class InfrastructureMamaLogger
  extends webpack.ProgressPlugin
  implements Logger
{
  static logName = c`{green.bold ARPON} {green.dim}`;
  ...
  async apply(compiler: webpack.Compiler) {
    ...
    hook(
      "infrastructureLog",
      (source: unknown, type: unknown, ...logs: unknown[]) => {
        let item = "";
        if (logs.length > 0) {
        ...
        if (source === "webpack-cli") {
          ...
          if (msg.match(/File '.*'/g)) {
            msg = msg.replace(
              /(File) ('.*') (.+)/g,
              c`{cyan $1} {magenta.bold $2} {cyan $3}`,
            );
          }
  ...
  private detectUrls(
    ...
    | { version: "network-v4" | "network-v6" | "local"; value: IpURL }
    | undefined {
  ...
  private parseRequest(requestStr: string | undefined) {
    ...
    const file = path.relative(
      ...
      this.removeAfter(
        "?",
        this.removeBefore(NODE_MODULES, parts.pop() as string),
      ),
    );
  ...
  private fillGap(str1: string, str2: string, offset = 0) {
    const repeatCount = Math.max(
      0,
      MAX_LINE_LENGTH -
        (str1.replace(onlyTextRegx, "").length +
          str2.replace(onlyTextRegx, "").length) -
        -offset,
    );
  ...
  private printServerUrls(urls: Urls, serverReady = false): string {
    ...
    if (
      Array.isArray(urls.network) &&
      urls.network.length > 0 &&
      urls.network[0] instanceof URL
    ) {
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
      const types = (url: IpURL) =>
        `${url.isIpv6 && urls.network!.length > 1 ? "IPv6" : "IPv4"}:`;

      urls.network.forEach((url: IpURL | string, index: number) => {
        const type = url instanceof URL ? types(url) : url;
        let line =
          url instanceof URL ? networkAddressLine(url.href, type) : url;

        const microGap = " ".repeat(
          Math.max(
            1,
            MAX_URL_LENGTH -
              (line.replace(onlyTextRegx, "").length - type.length) +
              1,
          ),
        );
  ...
  private handleProgress(percent = 0, message = "", details: string[] = []) {
    ...
    if (progress >= 0 && progress < 100) {
      ...
      line1 = this.fillGap(
        leftCol,
        `${progressText}${this.renderBar(progress, colorName)}`,
      );
